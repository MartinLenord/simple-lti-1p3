window.game = function(curr_user_name) {
    let for_user = false;
    ...
    window.game_recorder = new recording();
    ...
    var frame = function() {
        ...
        window.asteroids.render(ctx);
        window.spaceships.render(ctx);
        window.player.render(ctx);

        window.score_div.innerHTML = 'Score: ' + window.score;
        ...
        if (pause) {
            ...
            if (window.game_recorder.frame_count % 10 == 1) {
                window.fps_div.innerText = 'FPS: ' + Math.floor(1000/frame_time);
            }
            window.last_frame_time = Date.now();
